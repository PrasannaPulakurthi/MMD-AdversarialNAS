...
for layer in ${layers[@]}; do
    ...
    sbatch --output=Results/%x_%j.out --error=Results/%x_%j.err --time=1-0:0:0 --job-name=comL${layer_id}frz --export=CHECKPOINT=$checkpoint,BU=$bu,LAYER=$layer,R=$R,EXPNAME=$EXPNAME,MAXEPOCH=$maxepoch,FREEZE_LAYERS=$layer rc_scripts/compress_single_layer_nofreeze_template.sh
done
